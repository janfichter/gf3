...
/**
 * ИСПРАВЛЕНО (PHP 8): безопасная обёртка над sanitize_text_field().
 * sanitize_text_field() объявлена с типом параметра `string`. Если значение
 * из $_POST/$_GET/$_REQUEST окажется массивом (что легко подделать в запросе,
 * например nonce[]=1), прямой вызов sanitize_text_field($value) приводит
 * к неперехваченной фатальной ошибке TypeError. Эта функция гарантированно
 * возвращает строку в любом случае.
 */
if (!function_exists('family_tree_sanitize_str')) {
    function family_tree_sanitize_str($value) {
        if (!is_scalar($value)) {
            return '';
        }
...
<span class="checkbox-title"><?php esc_html_e('Сделать корневым элементом древа', 'genius-family-tree'); ?></span>
</label>
</div>
</fieldset>
<?php
...
return;
    ...
    // Проверяем nonce для быстрого редактирования
    ...
    $is_root = isset($_POST['family_member_is_root']) && sanitize_text_field(wp_unslash($_POST['family_member_is_root'])) == '1';
if ($is_root) {
...
// ИСПРАВЛЕНО: hook 'quick_edit_custom_box' уже зарегистрирован в конструкторе,
// повторная регистрация здесь приводила к двойному выводу чекбокса в Quick Edit.
}
/**
...
if (!empty($birth_date) && $birth_date !== $birth_year) {
        $tooltip[] = sprintf(
            /* translators: %s: formatted birth date */
            __('Родился(ась): %s', 'genius-family-tree'),
            $this->format_date_for_display($birth_date)
        );
}
if (!empty($death_date) && $death_date !== $death_year) {
        $tooltip[] = sprintf(
            /* translators: %s: formatted death date */
            __('Умер(ла): %s', 'genius-family-tree'),
            $this->format_date_for_display($death_date)
        );
}
if (!empty($tooltip)) {
...
echo esc_html($group->post_title);
        ...
        echo '<em style="color: #999;">' . esc_html__('Группа не найдена', 'genius-family-tree') . '</em>';
}
} else {
        echo '<em style="color: #999;">' . esc_html__('Без группы', 'genius-family-tree') . '</em>';
}
}
/**
...
* ИСПРАВЛЕНО: раньше подсчёт шёл через get_posts()/WP_Query, что на этом
* сайте давало ненадёжный результат (см. аналогичный фикс в count_ungrouped_members()).
* Теперь используется прямой SQL-запрос через $wpdb, как и там.
*/
...
WHERE p.post_type = 'family_member'
AND p.post_status NOT IN ('trash', 'auto-draft')
AND pm.meta_key = '_family_member_group_id'
AND pm.meta_value = %s
", (string) $group_id));
}
/**
...
* Используется WP_Query с posts_per_page => -1 и suppress_filters => true,
* чтобы гарантированно получить все записи и игнорировать
* сторонние фильтры, ограничивающие вывод.
* ИСПРАВЛЕНО: поиск раньше выполнялся через встроенный параметр WP_Query 's',
* который ищет не только по имени, но и по всему содержимому записи
* (post_content/post_excerpt) — из-за этого в результатах поиска "Яков"
* появлялись персоналии, у которых слово "Яков" просто где-то упоминается
* в тексте биографии, а не в имени. Теперь поиск ведётся только по полям
* имени/отчества/фамилии/девичьей фамилии и по заголовку записи.
*/
...
// Получаем параметры из GET запроса
    ...
    // phpcs:ignore WordPress.Security.NonceVerification.Recommended
    $sort_order = isset($_GET['ft_group_sort']) ? sanitize_text_field(wp_unslash($_GET['ft_group_sort'])) : 'date_asc';

// Строим запрос с явными параметрами для получения ВСЕХ записей группы
// (без учёта поиска — поиск теперь применяется отдельно, ниже, по PHP-стороне
// только к полям имени, а не ко всему содержимому записи)
$query_args = array(
'post_type' => 'family_member',
'posts_per_page' => -1, // Получить все записи
'nopaging' => true,     // Отключить разбиение на страницы
        'ignore_sticky_posts' => 1, // Игнорировать закрепленные записи
        'post_status' => 'any',
        // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_query
        'meta_query' => array(
array(
...
switch ($sort_order) {
    case 'date_asc':
        // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
        $query_args['meta_key'] = '_family_member_birth_date';
...
// Выполняем запрос — получаем ВСЕХ членов группы (это и есть надёжное
// "общее количество персоналий" группы, не зависящее от поиска)
$query = new WP_Query($query_args);
$all_group_members = $query->posts;
$total_group_count = count($all_group_members);

// ИСПРАВЛЕНО: поиск теперь фильтрует уже полученный список персоналий
// группы по именным полям (и заголовку), а не по всему содержимому записи.
// Порядок, заданный сортировкой выше, при этом сохраняется.
if (!empty($search_query)) {
$members = array_values(array_filter($all_group_members, function($member) use ($search_query) {
$haystacks = array(
$member->post_title,
get_post_meta($member->ID, '_family_member_first_name', true),
get_post_meta($member->ID, '_family_member_middle_name', true),
get_post_meta($member->ID, '_family_member_last_name', true),
get_post_meta($member->ID, '_family_member_maiden_name', true),
);
foreach ($haystacks as $haystack) {
if ($haystack !== '' && mb_stripos((string) $haystack, $search_query, 0, 'UTF-8') !== false) {
return true;
}
}
return false;
...
<p class="ft-member-count" style="font-weight: 600; margin: 0 0 10px;">
<?php if (!empty($search_query)): ?>
<?php
printf(
/* translators: 1: найдено, 2: всего в группе */
esc_html__('Найдено: %1$d из %2$d персоналий в группе', 'genius-family-tree'),
count($members),
esc_html($total_group_count)
);
?>
<?php else: ?>
<?php
/* translators: %d: total number of persons in group */
printf(esc_html__('Всего в группе: %d персоналий', 'genius-family-tree'), esc_html($total_group_count)); ?>
<?php endif; ?>
</p>
<div class="tablenav top ft-controls-bar" id="ft-controls-bar">
<div class="alignleft actions">
<label class="screen-reader-text" for="ft-group-sort"><?php esc_html_e('Сортировка', 'genius-family-tree'); ?></label>
<select name="ft_group_sort" id="ft-group-sort">
...
<button type="button" class="button action ft-btn-apply"><?php esc_html_e('Применить', 'genius-family-tree'); ?></button>
</div>
<div class="alignleft actions">
...
<?php
if (empty($members)) {
    echo '<p style="margin: 12px 0 0; color: #646970;">' . esc_html__('В этой группе пока нет персоналий', 'genius-family-tree') . '</p>';
echo '</div></div>';
return;
}
?>
...
<thead>
<tr>
<th scope="col" class="manage-column column-title column-primary"><?php esc_html_e('Имя', 'genius-family-tree'); ?></th>
<th scope="col" class="manage-column"><?php esc_html_e('Портрет', 'genius-family-tree'); ?></th>
<th scope="col" class="manage-column"><?php esc_html_e('Годы жизни', 'genius-family-tree'); ?></th>
<th scope="col" class="manage-column"><?php esc_html_e('Корень древа', 'genius-family-tree'); ?></th>
</tr>
</thead>
<tbody>
...
$is_root = get_post_meta($member_id, '_family_member_is_root', true);
?>
<tr>
<td class="title column-title has-row-actions column-primary" data-colname="<?php esc_attr_e('Имя', 'genius-family-tree'); ?>">
<strong>
...
<span class="edit">
<a href="<?php echo esc_url($edit_link); ?>"><?php esc_html_e('Редактировать', 'genius-family-tree'); ?></a>
</span>
</div>
</td>
<td data-colname="<?php esc_attr_e('Портрет', 'genius-family-tree'); ?>">
<?php $this->display_portrait_column($member_id); ?>
</td>
<td data-colname="<?php esc_attr_e('Годы жизни', 'genius-family-tree'); ?>">
<?php $this->display_lifespan_column($member_id); ?>
</td>
<td data-colname="<?php esc_attr_e('Корень древа', 'genius-family-tree'); ?>">
<?php
if ($is_root) {
echo '<span class="dashicons dashicons-star-filled" style="color: #ffb900;" title="' . esc_attr__('Корневой элемент древа', 'genius-family-tree') . '"></span>';
} else {
...
// Получаем текущие параметры
    // phpcs:ignore WordPress.Security.NonceVerification.Recommended
    $sort_order = isset($_GET['ft_member_sort']) ? sanitize_text_field(wp_unslash($_GET['ft_member_sort'])) : 'date_asc';
?>
<style>
.ft-member-sort-form {
display: inline-flex;
...
<!-- Форма сортировки -->
<span class="ft-member-sort-form">
...
// Сортировка
    ...
    if (isset($_GET['ft_member_sort'])) {
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended
        $sort_order = sanitize_text_field(wp_unslash($_GET['ft_member_sort']));
switch ($sort_order) {
case 'date_asc':
// phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
...
// Добавляем подменю "Добавить персону"
add_submenu_page(
'family-tree-main',
...
/**
* ИСПРАВЛЕНО: Надежный подсчет через $wpdb вместо get_posts
*/
    private function count_ungrouped_members() {
        global $wpdb;
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
        return (int) $wpdb->get_var($wpdb->prepare("
            SELECT COUNT(DISTINCT p.ID)
            ...
            LEFT JOIN {$wpdb->posts} g ON g.ID = pm.meta_value AND g.post_type = 'family_group' AND g.post_status != 'trash'
            WHERE p.post_type = 'family_member'
            AND p.post_status != 'trash'
            AND (pm.meta_value IS NULL OR pm.meta_value = '' OR pm.meta_value = '0' OR g.ID IS NULL)
        ", '_family_member_group_id'));
    }
    public function main_page() {
        ...
        $total_groups = (int) $wpdb->get_var($wpdb->prepare("
            ...
            AND post_status IN ('publish', 'draft', 'pending', 'private')
        ", 'family_group'));
        $ungrouped_count = $this->count_ungrouped_members();
        $ungrouped_members = array();
        if ($ungrouped_count > 0) {
            global $wpdb;
            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
            $ungrouped_members = $wpdb->get_results($wpdb->prepare("
        ...
        $root_count = (int) $wpdb->get_var("
            ...
            FROM {$wpdb->postmeta} pm
            INNER JOIN {$wpdb->posts} p ON pm.post_id = p.ID
            WHERE pm.meta_key = '_family_member_is_root'
            AND pm.meta_value = '1'
            AND p.post_type = 'family_member'
            AND p.post_status != 'trash'
        ");
        ...
        <div class="wrap ft-dashboard">
            <div class="ft-version-banner">
                ...
                <div class="ft-version-actions">
                    <a href="<?php echo esc_url(admin_url('admin.php?page=family-tree-settings')); ?>" class="button"><?php echo esc_html__('Настройки', 'genius-family-tree'); ?></a>
                    <?php if (!$is_pro): ?>
            ...
            <div class="notice notice-warning ft-notice">
                <p>
                    <strong><?php echo esc_html__('Внимание!', 'genius-family-tree'); ?></strong><br>
                    <?php printf(
                        /* translators: %d: number of ungrouped members */
                        esc_html(
                            /* translators: %d: number of ungrouped members */
                            _n(
                                'Найдена %d персоналия без семейной группы. Не забудьте перенести её в нужную группу.',
                                'Найдено %d персоналий без семейной группы. Не забудьте перенести их в нужную группу.',
                                $ungrouped_count,
                                'genius-family-tree'
                            )
                        ),
                        esc_html($ungrouped_count)
                    ); ?>
                </p>
                <?php if (!empty($ungrouped_members)): ?>
                <ul style="margin: 10px 0 0 18px; padding: 0;">
                    <?php foreach ($ungrouped_members as $member): ?>
                    <li>
                        ...
                        <span style="color: #646970; font-size: 12px;">
                            ...
                            <?php if ($member->post_status !== 'publish'): ?>
                                — <?php echo esc_html(get_post_status_object($member->post_status)->label); ?>
                            <?php endif; ?>
                        </span>
                    </li>
                    <?php endforeach; ?>
                    <?php if ($ungrouped_count > 20): ?>
                    <li style="color: #646970; font-style: italic;">
                        <?php echo esc_html__('И ещё...', 'genius-family-tree'); ?>
                    </li>
                    <?php endif; ?>
                </ul>
                <?php endif; ?>
            ...
            <div class="ft-tiles">
                <a href="<?php echo esc_url(admin_url('edit.php?post_type=family_member')); ?>" class="ft-tile">
                    <div class="ft-tile-icon dashicons-before dashicons-groups"></div>
                    <div class="ft-tile-content">
                        <strong><?php echo esc_html__('Все персоналии', 'genius-family-tree'); ?></strong>
                        <span class="ft-tile-count"><?php echo esc_html($total_members); ?></span>
                    </div>
                </a>
                <a href="<?php echo esc_url(admin_url('edit.php?post_type=family_group')); ?>" class="ft-tile">
                    <div class="ft-tile-icon dashicons-before dashicons-category"></div>
                    <div class="ft-tile-content">
                        <strong><?php echo esc_html__('Группы', 'genius-family-tree'); ?></strong>
                        <span class="ft-tile-count"><?php echo esc_html($total_groups); ?></span>
                    </div>
                </a>
                <a href="<?php echo esc_url(admin_url('post-new.php?post_type=family_member')); ?>" class="ft-tile ft-tile-add">
                    <div class="ft-tile-icon dashicons-before dashicons-plus-alt"></div>
                    <div class="ft-tile-content">
                        <strong><?php echo esc_html__('Добавить персону', 'genius-family-tree'); ?></strong>
                        <span class="ft-tile-hint"><?php echo esc_html__('Новая запись', 'genius-family-tree'); ?></span>
                    </div>
                </a>
                <a href="<?php echo esc_url(admin_url('post-new.php?post_type=family_group')); ?>" class="ft-tile ft-tile-add">
                    <div class="ft-tile-icon dashicons-before dashicons-plus-alt2"></div>
                    <div class="ft-tile-content">
                        <strong><?php echo esc_html__('Новая группа', 'genius-family-tree'); ?></strong>
                        <span class="ft-tile-hint"><?php echo esc_html__('Новое древо', 'genius-family-tree'); ?></span>
                    </div>
                </a>
            </div>

            <div class="ft-status-block">
                <h3><?php echo esc_html__('Состояние', 'genius-family-tree'); ?></h3>
                <div class="ft-status-items">
                    <div class="ft-status-item">
                        <span class="ft-status-label"><?php echo esc_html__('Без группы', 'genius-family-tree'); ?></span>
                        <span class="ft-status-value <?php echo $ungrouped_count > 0 ? 'ft-status-warning' : 'ft-status-ok'; ?>"><?php echo esc_html($ungrouped_count); ?></span>
                    </div>
                    <div class="ft-status-item">
                        <span class="ft-status-label"><?php echo esc_html__('Корневые элементы', 'genius-family-tree'); ?></span>
                        <span class="ft-status-value ft-status-ok"><?php echo esc_html($root_count); ?></span>
                    </div>
                    <div class="ft-status-item">
                        <span class="ft-status-label"><?php echo esc_html__('Всего персоналий', 'genius-family-tree'); ?></span>
                        <span class="ft-status-value"><?php echo esc_html($total_members); ?></span>
                    </div>
                </div>
            </div>

            <div class="ft-dashboard-cols">
                <div class="ft-col">
                    <div class="ft-card">
                        <h3><?php echo esc_html__('Последние действия', 'genius-family-tree'); ?></h3>
                        <?php if (!empty($recent_members)): ?>
                        <ul class="ft-recent-list">
                            <?php foreach ($recent_members as $member):
                                $edit_link = get_edit_post_link($member->ID);
                                $date = get_the_date('d.m.Y H:i', $member->ID);
                                $status = get_post_status($member->ID);
                                $status_label = $status === 'publish' ? __('Опубликовано', 'genius-family-tree') : __('Черновик', 'genius-family-tree');
                            ?>
                            <li>
                                <a href="<?php echo esc_url($edit_link); ?>" class="ft-recent-title"><?php echo esc_html(get_the_title($member->ID)); ?></a>
                                <span class="ft-recent-meta"><?php echo esc_html($date); ?> · <?php echo esc_html($status_label); ?></span>
                                <div class="row-actions"><span class="edit"><a href="<?php echo esc_url($edit_link); ?>"><?php echo esc_html__('Редактировать', 'genius-family-tree'); ?></a></span></div>
                            </li>
                            <?php endforeach; ?>
                        </ul>
                        <?php else: ?>
                        <p><?php echo esc_html__('Пока нет записей', 'genius-family-tree'); ?></p>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div class="ft-col">
                    <div class="ft-card">
                        <h3><?php echo esc_html__('Быстрый поиск персоны', 'genius-family-tree'); ?></h3>
                        <form method="get" action="<?php echo esc_url(admin_url('edit.php')); ?>" class="ft-quick-search-form">
                            <input type="hidden" name="post_type" value="family_member">
                            <input type="text" name="s" placeholder="<?php echo esc_attr__('Имя или фамилия...', 'genius-family-tree'); ?>" class="regular-text" required>
                            <button type="submit" class="button button-primary"><?php echo esc_html__('Найти', 'genius-family-tree'); ?></button>
                        </form>
                    </div>
            ...
            <div class="ft-card ft-wizard-card">
                <h3><?php echo esc_html__('Мастер запуска', 'genius-family-tree'); ?></h3>
                <div class="ft-wizard-steps">
                    <div class="ft-wizard-step">
                        <div class="ft-wizard-num">1</div>
                        <div class="ft-wizard-text">
                            <strong><?php echo esc_html__('Создайте семейную группу', 'genius-family-tree'); ?></strong>
                            <p><?php echo esc_html__('Группа — это контейнер для одного древа. Например, «Родовод Фёдоровых».', 'genius-family-tree'); ?></p>
                            <a href="<?php echo esc_url(admin_url('post-new.php?post_type=family_group')); ?>" class="button button-small"><?php echo esc_html__('Создать группу', 'genius-family-tree'); ?></a>
                        </div>
                    </div>
                    <div class="ft-wizard-step">
                        <div class="ft-wizard-num">2</div>
                        <div class="ft-wizard-text">
                            <strong><?php echo esc_html__('Добавьте первую персону', 'genius-family-tree'); ?></strong>
                            <p><?php echo esc_html__('Заполните имя, годы жизни, выберите группу и загрузите портрет.', 'genius-family-tree'); ?></p>
                            <a href="<?php echo esc_url(admin_url('post-new.php?post_type=family_member')); ?>" class="button button-small"><?php echo esc_html__('Добавить персону', 'genius-family-tree'); ?></a>
                        </div>
                    </div>
                    <div class="ft-wizard-step">
                        <div class="ft-wizard-num">3</div>
                        <div class="ft-wizard-text">
                            <strong><?php echo esc_html__('Укажите корневой элемент', 'genius-family-tree'); ?></strong>
                            <p><?php echo esc_html__('В правом столбце редактирования персоны поставьте галочку «Корневой элемент древа».', 'genius-family-tree'); ?></p>
                        </div>
                    </div>
                    <div class="ft-wizard-step">
                        <div class="ft-wizard-num">4</div>
                        <div class="ft-wizard-text">
                            <strong><?php echo esc_html__('Вставьте шорткод на страницу', 'genius-family-tree'); ?></strong>
                            <p><?php echo esc_html__('Скопируйте шорткод из списка групп и вставьте в текст страницы или записи.', 'genius-family-tree'); ?></p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        .ft-tile-icon { font-size: 32px; color: #2271b1; width: 48px; height: 48px; display: flex; align-items: center; justify-content: center; background: #f0f6fc; border-radius: 4px; }
        .ft-tile-add .ft-tile-icon { background: #f0fdf4; color: #008a20; }
        .ft-tile-content { display: flex; flex-direction: column; gap: 2px; }
        .ft-tile-count { font-size: 22px; font-weight: 600; color: #1d2327; }
        .ft-tile-hint { font-size: 12px; color: #646970; }
        .ft-status-block { background: #fff; border: 1px solid #c3c4c7; padding: 16px 20px; margin: 0 0 20px; border-radius: 4px; }
        .ft-status-block h3 { margin: 0 0 12px; font-size: 14px; text-transform: uppercase; color: #50575e; }
        .ft-status-items { display: flex; gap: 32px; flex-wrap: wrap; }
        .ft-status-item { display: flex; flex-direction: column; gap: 4px; }
        .ft-status-label { font-size: 12px; color: #646970; }
        .ft-status-value { font-size: 20px; font-weight: 600; color: #1d2327; }
        .ft-status-warning { color: #d63638; }
        .ft-status-ok { color: #00a32a; }
        .ft-dashboard-cols { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin: 20px 0; }
        @media (max-width: 782px) { .ft-dashboard-cols { grid-template-columns: 1fr; } }
        .ft-card { background: #fff; border: 1px solid #c3c4c7; padding: 20px; border-radius: 4px; }
        .ft-card h3 { margin: 0 0 14px; font-size: 14px; text-transform: uppercase; color: #50575e; }
        .ft-recent-list { list-style: none; margin: 0; padding: 0; }
        .ft-recent-list li { padding: 10px 0; border-bottom: 1px solid #f0f0f1; }
        .ft-recent-list li:last-child { border-bottom: none; }
        .ft-recent-title { font-weight: 600; color: #2271b1; text-decoration: none; }
        .ft-recent-title:hover { color: #135e96; }
        .ft-recent-meta { display: block; font-size: 12px; color: #646970; margin-top: 2px; }
        ...
        .ft-wizard-step { display: flex; gap: 14px; }
        .ft-wizard-num { width: 36px; height: 36px; border-radius: 50%; background: #2271b1; color: #fff; display: flex; align-items: center; justify-content: center; font-weight: 600; flex-shrink: 0; }
        .ft-wizard-text { flex: 1; }
        .ft-wizard-text strong { display: block; margin-bottom: 4px; }
        .ft-wizard-text p { margin: 0 0 8px; font-size: 13px; color: #50575e; }
        </style>
        <?php
    }
/**
...
?>
<div class="wrap">
<h1><?php esc_html_e('Миграция персоналий', 'genius-family-tree'); ?></h1>
<div class="notice notice-error">
<p><?php esc_html_e('Группа по умолчанию не найдена. Пожалуйста, активируйте плагин заново или создайте группу вручную.', 'genius-family-tree'); ?></p>
</div>
</div>
<?php
return;
...
$migrated = $this->migrate_ungrouped_members_to_default($default_group_id);
?>
<div class="wrap">
<div class="notice notice-success">
<p><?php printf(
    /* translators: %d: number of migrated persons */
    esc_html__('Успешно перенесено %d персоналий в группу «Без названия».', 'genius-family-tree'),
    esc_html($migrated)
); ?></p>
</div>
<p><a href="<?php echo esc_url(admin_url('edit.php?post_type=family_member')); ?>" class="button"><?php esc_html_e('Вернуться к списку персоналий', 'genius-family-tree'); ?></a></p>
</div>
<?php
return;
}
...
<p><?php printf(
    ...
    esc_html__('Эти персоналии будут перенесены в группу «%s». Вы сможете изменить название группы или перераспределить персоналий позже.', 'genius-family-tree'),
...
<a href="<?php echo esc_url(admin_url('edit.php?post_type=family_member')); ?>" class="button button-secondary" style="margin-left: 10px;"><?php esc_html_e('Отмена', 'genius-family-tree'); ?></a>
</form>
<?php else: ?>
<div class="notice notice-success">
<p><?php esc_html_e('Все персоналии уже распределены по группам. Миграция не требуется.', 'genius-family-tree'); ?></p>
</div>
<p><a href="<?php echo esc_url(admin_url('edit.php?post_type=family_member')); ?>" class="button"><?php esc_html_e('Вернуться к списку персоналий', 'genius-family-tree'); ?></a></p>
<?php endif; ?>
</div>
...
/**
* ИСПРАВЛЕНО: Надежная миграция через прямой SQL + очистка кэша
*/
private function migrate_ungrouped_members_to_default($default_group_id) {
    global $wpdb;
    $default_group_id = intval($default_group_id);
    if (!$default_group_id) return 0;
    // Находим все ID персоналий без группы (NULL, '' или '0')
    // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
    $ids = $wpdb->get_col("
SELECT DISTINCT p.ID
FROM {$wpdb->posts} p
LEFT JOIN {$wpdb->postmeta} pm ON p.ID = pm.post_id AND pm.meta_key = '_family_member_group_id'
WHERE p.post_type = 'family_member'
AND p.post_status != 'trash'
...
$updated = 0;
foreach ($ids as $id) {
update_post_meta($id, '_family_member_group_id', $default_group_id);
wp_cache_delete($id, 'post_meta');
$updated++;
}
// Сбрасываем общий кэш запросов
wp_cache_flush();
return $updated;
}
...
// ИСПРАВЛЕНО (PHP 8): sanitize_text_field() требует строку. Если значение
// является массивом/bool/null (например, вложенные данные из ответа API),
// прямой вызов sanitize_text_field($value) приводил к фатальной ошибке TypeError.
if (is_array($value)) {
$sanitized[$safe_key] = $this->sanitize_license_data($value);
...
Family_Tree_Shortcode::init();
    ...
    private function add_performance_indexes() {
        ...
        $table = $wpdb->postmeta;
        $index_map = array(
            '_family_member_group_id' => 'idx_ft_group_id',
            '_family_member_is_root' => 'idx_ft_is_root',
            '_family_member_father' => 'idx_ft_father',
            '_family_member_mother' => 'idx_ft_mother',
        );
        foreach ($index_map as $meta_key => $index_name) {
            ...
            if (empty($exists)) {
                ...
                $wpdb->query($wpdb->prepare("ALTER TABLE `$table` ADD INDEX `$index_name` (`meta_key`(191), `meta_value`(20))"));
            }
        }
    }
/**
...
/**
* Находит или создает семейную группу по названию
*/
private function get_or_create_family_group_by_name($group_name) {
...
if (!empty($existing_groups)) {
return (int) $existing_groups[0]->ID;
}
$group_id = wp_insert_post(array(
'post_type' => 'family_group',
...
<div class="wrap ft-settings-page">
    ...
    <div class="ft-settings-layout">
            ...
            <form method="post" action="">
                ...
                <table class="form-table">
                    <tr>
                        ...
                        <td>
                            <input type="text" name="family_tree_license_key" value="<?php echo esc_attr($license_key); ?>" class="regular-text" placeholder="<?php echo esc_attr__('Введите лицензионный ключ', 'genius-family-tree'); ?>" />
                            ...
                            <div class="license-info">
                                <strong><?php echo esc_html__('Информация о лицензии:', 'genius-family-tree'); ?></strong>
                                <div class="license-info-row">
                                    <?php if (isset($license_data['customer_name'])): ?>
                                    <span><?php echo esc_html__('Владелец:', 'genius-family-tree'); ?> <strong><?php echo esc_html($license_data['customer_name']); ?></strong></span>
                                    <?php endif; ?>
                                    <?php if (isset($license_data['license_tier']) && $license_data['license_tier'] === 'basic' && isset($license_data['expires'])): 
                                        $expires_timestamp = strtotime($license_data['expires']);
                                        if ($expires_timestamp && $expires_timestamp > 0): ?>
                                    <span><?php echo esc_html__('Действует до:', 'genius-family-tree'); ?> <strong><?php echo esc_html(gmdate('d.m.Y', $expires_timestamp)); ?></strong></span>
                                    <?php endif; endif; ?>
                                    <?php if (isset($license_data['license_tier'])): ?>
                                    <span><?php echo esc_html__('Тип:', 'genius-family-tree'); ?> <strong><?php
                                    ...
                                    ?></strong></span>
                                    <?php endif; ?>
                                    <span><?php echo esc_html__('Последняя проверка:', 'genius-family-tree'); ?> <?php echo esc_html(gmdate('d.m.Y H:i', $last_check)); ?></span>
                                </div>
                            </div>
                            <?php endif; ?>
                        </td>
                    ...
                    <tr>
                        <th scope="row"><?php echo esc_html__('Статус лицензии', 'genius-family-tree'); ?></th>
                        <td>
        ...
        <div class="ft-card">
            ...
            <?php if ($has_gedcom): ?>
            <div class="gedcom-buttons-section">
                <div class="gedcom-group-selector">
                    <label for="gedcom_group_id"><?php echo esc_html__('Семейная группа:', 'genius-family-tree'); ?></label>
                    <select name="gedcom_group_id" id="gedcom_group_id">
                    ...
                    <p class="description"><?php echo esc_html__('Выберите группу для экспорта/импорта. Если не выбрано, операция будет выполнена для всех групп.', 'genius-family-tree'); ?></p>
                </div>
                <div class="gedcom-button-wrapper">
                    <a href="#" class="button button-primary button-hero" id="gedcom-export-btn" onclick="return false;">
                    ...
                    jQuery(document).ready(function($) {
                        $('#gedcom-export-btn').on('click', function() {
                            ...
                            var nonce = '<?php echo esc_js(wp_create_nonce('family_tree_gedcom')); ?>';
                            var url = '<?php echo esc_url(admin_url('admin-ajax.php?action=family_tree_export_gedcom&_wpnonce=')); ?>' + nonce;
                ...
                <div class="gedcom-button-wrapper">
                    <form method="post" action="<?php echo esc_url(admin_url('admin-ajax.php')); ?>" enctype="multipart/form-data" class="gedcom-import-form">
                        ...
                        <input type="file" name="gedcom_file" accept=".ged,.GED" required class="gedcom-file-input" />
                        <input type="submit" class="button button-secondary button-hero gedcom-submit" value="<?php echo esc_attr__('Импорт из GEDCOM', 'genius-family-tree'); ?>" />
        ...
        <?php else: ?>
        <div class="ft-card">
            <div class="ft-card-header"><?php echo esc_html__('Доступные функции в Pro-версии', 'genius-family-tree'); ?></div>
            <ul class="ft-features-list">
            ...
            <div class="ft-upgrade-cta">
                <a href="https://xn----8sbbdpda1c7cwf.xn--p1ai/product-category/genius-family-tree/" class="button button-primary button-hero" target="_blank"><?php echo esc_html__('Приобрести Pro-версию', 'genius-family-tree'); ?></a>
            </div>
        </div>
        <?php endif; ?>
    </div>
...
.ft-settings-page .ft-card {
    max-width: 900px;
}
.ft-settings-page .ft-card-header {
    font-size: 17px;
    font-weight: 700;
    color: #135e96;
    margin: 0 0 24px;
...
.ft-settings-page .ft-card-header:before {
    content: "";
...
.gedcom-button-wrapper {
    margin-bottom: 12px;
}
.gedcom-button-wrapper .button {
    width: 100%;
    text-align: center;
    padding: 10px;
    font-size: 14px;
...
$used_families = array();
$family_counter = 1;
// Находим корневой элемент (если есть), с учетом выбранной группы
$root_member_id = $this->get_root_member_id($group_id);
...
$member_group_id = (int) get_post_meta($id, '_family_member_group_id', true);
$member_group_name = '';
$member_group = null;
$is_root = ($root_member_id == $id);
$member_group = $member_group_id ? get_post($member_group_id) : null;
if ($member_group && $member_group->post_type === 'family_group') {
$member_group_name = sanitize_text_field($member_group->post_title);
}
...
$gedcom_content .= "1 _GROUP " . $member_group_name . "\n";
}
...
if (!isset($individuals[$spouse_id])) continue;
...
echo wp_kses($gedcom_content, array());
exit;
}
/**
...
$group_id = isset($_POST['gedcom_group_id']) ? intval(wp_unslash($_POST['gedcom_group_id'])) : 0;
...
if (isset($_FILES['gedcom_file']['error']) && $_FILES['gedcom_file']['error'] !== UPLOAD_ERR_OK) {
    wp_die(esc_html__('Ошибка загрузки файла', 'genius-family-tree'));
}
$file_extension = isset($_FILES['gedcom_file']['name']) ? strtolower(sanitize_text_field(wp_unslash($_FILES['gedcom_file']['name']))) : '';
...
if (isset($_FILES['gedcom_file']['tmp_name']) && is_uploaded_file($_FILES['gedcom_file']['tmp_name'])) {
    // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
...
<?php
/* translators: %s: number of imported persons */
echo sprintf(
    esc_html(
        /* translators: %s: number of imported persons */
        _n(
    ...
    '<strong>' . esc_html(number_format_i18n($import_result['persons_imported'])) . '</strong>'
);
?>
</p>
...
<?php
/* translators: %s: number of imported images */
echo sprintf(
    esc_html(
        /* translators: %s: number of imported images */
        _n(
    ...
    '<strong>' . esc_html(number_format_i18n($import_result['images_imported'])) . '</strong>'
); ?>
</p>
<?php endif; ?>
...
<?php
/* translators: %s: root member name */
echo sprintf(
    /* translators: %s: root member name */
    esc_html__('Корневой элемент древа: %s', 'genius-family-tree'),
...
<p style="margin: 0; font-size: 15px;">
<?php echo esc_html__('Количество ошибок:', 'genius-family-tree'); ?> <strong><?php echo esc_html(number_format_i18n($import_result['errors_count'])); ?></strong>
</p>
...
break;
case '_GROUP':
$current_individual['group_name'] = $value;
break;
}
} elseif ($level == 2) {
...
private function import_gedcom_data($parsed_data) {
    ...
    $group_id = isset($_POST['gedcom_group_id']) ? intval(sanitize_text_field(wp_unslash($_POST['gedcom_group_id']))) : 0;
$result = array(
'persons_imported' => 0,
'images_imported' => 0,
'errors_count' => 0,
...
$resolved_group_ids = array();
$default_group_id = (int) get_option('family_tree_default_group_id', 0);
...
// Если группа выбрана в форме импорта, она имеет приоритет.
// Иначе пробуем восстановить группу из GEDCOM, а при её отсутствии используем группу по умолчанию.
$resolved_group_id = 0;
if ($group_id) {
$resolved_group_id = $group_id;
} elseif (!empty($individual['group_name'])) {
$normalized_group_name = sanitize_text_field($individual['group_name']);
if ($normalized_group_name !== '') {
if (!isset($resolved_group_ids[$normalized_group_name])) {
$resolved_group_ids[$normalized_group_name] = $this->get_or_create_family_group_by_name($normalized_group_name);
}
$resolved_group_id = (int) $resolved_group_ids[$normalized_group_name];
}
} elseif ($default_group_id && get_post($default_group_id)) {
$resolved_group_id = $default_group_id;
}
if ($resolved_group_id) {
$post_data['meta_input']['_family_member_group_id'] = $resolved_group_id;
}
$post_id = wp_insert_post($post_data, true);
if (is_wp_error($post_id)) {
$result['errors'][] = sprintf(
    /* translators: 1: person name, 2: error message */
    __('Ошибка создания персоны "%1$s": %2$s', 'genius-family-tree'),
    esc_html($individual['name']),
...
// Сбрасываем существующий корневой элемент только в пределах определенной группы.
// Если группа не определена, не трогаем остальные записи сайта.
if ($root_group_id) {
...
'timeout' => 30,
'stream' => false,
// ИСПРАВЛЕНО (безопасность): не отключаем проверку SSL-сертификата —
// это открывало возможность атаки "человек посередине" при скачивании
// изображений по адресам из импортируемого GEDCOM-файла.
'sslverify' => true
));
...
$member_count = $count->publish;
if ($member_count >= 50) {
// ИСПРАВЛЕНО: save_post срабатывает уже ПОСЛЕ вставки записи в базу,
// поэтому простой wp_die() оставлял "осиротевшую" опубликованную запись.
// Теперь превышающая лимит запись отправляется в корзину перед остановкой.
remove_action('save_post_family_member', array($this, 'check_member_limit_on_save'), 10);
